<?php
/** 
 *------------------------------------------------------------------------------
 * @package       Optimum Theme
 *------------------------------------------------------------------------------
 * @copyright     Copyright (C) 2013-2014 Optimum Theme. All Rights Reserved.
 * @license       Optimum Theme Property Licence
 * @authors       OptimumTheme.com
 * @Link:         http://www.optimumtheme.com
 *------------------------------------------------------------------------------
 */

defined('_JEXEC') or die;
?>

<?php if ($this->checkSpotlight('field-6', 'grid-31, grid-32, grid-33, grid-34')) : ?>
	<!-- FIELD 6 -->
	<div class="ot-field ot-field-6">
		<div class="container">
			<?php $this->spotlight('field-6', 'grid-31, grid-32, grid-33, grid-34') ?>
		</div>
	</div>
	<!-- //FIELD 6 -->
<?php endif ?>