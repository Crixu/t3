<?php
/** 
 *------------------------------------------------------------------------------
 * @package       Optimum Theme
 *------------------------------------------------------------------------------
 * @copyright     Copyright (C) 2013-2014 Optimum Theme. All Rights Reserved.
 * @license       Optimum Theme Property Licence
 * @authors       OptimumTheme.com
 * @Link:         http://www.optimumtheme.com
 *------------------------------------------------------------------------------
 */

defined('_JEXEC') or die;
?>

<?php if ($this->checkSpotlight('field-5', 'grid-25, grid-26, grid-27, grid-28')) : ?>
	<!-- FIELD 5 -->
	<div class="ot-field ot-field-5">
		<div class="container">
			<?php $this->spotlight('field-5', 'grid-25, grid-26, grid-27, grid-28') ?>
		</div>
	</div>
	<!-- //FIELD 5 -->
<?php endif ?>